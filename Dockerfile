...
COPY environment.yml /lcls-cu-inj-live/environment.yml

RUN apt-get update && \
    apt-get install -y gcc git build-essential

RUN mamba install -c conda-forge conda-pack && \
  mamba env create -f /lcls-cu-inj-live/environment.yml

# Use conda-pack to create a  enviornment in /venv:
RUN conda-pack -n lcls-cu-inj-live -o /tmp/env.tar && \
...
COPY lcls_cu_inj_live/flow.py /opt/prefect/flow.py

RUN chmod +x /usr/local/bin/_entrypoint.sh

RUN source /venv/bin/activate && \
  python -m pip install /lcls-cu-inj-live
